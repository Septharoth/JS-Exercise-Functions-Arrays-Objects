...
*/
function getModelYears(i) {
  let newArr = [];
  for (let a=0;a<i.length;a++) {
    newArr.push(i[a][`car_year`]);
  }
  return newArr;
}
...
function getGermanCars(inv) {
  let newArr = [];
  for (let a=0;a<inv.length;a++) {
    if(inv[a][`car_make`] == "Audi" || inv[a][`car_make`] ==  "Mercedes-Benz" || inv[a][`car_make`] ===  "Volkswagen" || inv[a][`car_make`] ===  "BMW") {
      newArr.push(inv[a]);
    }
  }
  return newArr;
